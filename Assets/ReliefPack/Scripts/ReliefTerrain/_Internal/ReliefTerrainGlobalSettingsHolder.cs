		...
		for(int i=0; i<rts.Length; i++) {
			if (rts[i].globalSettingsHolder!=null) {
				...
				if (ter) {
					...
					rts[i].globalSettingsHolder.Refresh(rts[i].renderer.sharedMaterial);
				}
		...
		if (mat==null && rt_caller!=null) {
			if (rt_caller.globalSettingsHolder==this) {
				...
				if (ter) {
					...
					if (rt_caller.renderer!=null && rt_caller.renderer.sharedMaterial!=null) {
						rt_caller.globalSettingsHolder.Refresh(rt_caller.renderer.sharedMaterial);
					}
				}
			}
		}

			...
			if (renderer) {
				if (lightForSelfShadowing.type==LightType.Directional) {
					for(int i=0; i<renderer.sharedMaterials.Length; i++) {
						renderer.sharedMaterials[i].SetVector("_WorldSpaceLightPosCustom", -lightForSelfShadowing.transform.forward);
					}
				} else {
					for(int i=0; i<renderer.materials.Length; i++) {
						renderer.sharedMaterials[i].SetVector("_WorldSpaceLightPosCustom", new Vector4(lightForSelfShadowing.transform.position.x, lightForSelfShadowing.transform.position.y, lightForSelfShadowing.transform.position.z, 1));
					}
				}
			} else {
				if (lightForSelfShadowing.type==LightType.Directional) {

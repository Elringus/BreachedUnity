...
//#define RTP_MAPPED_SHADOWS
	...
	#undef USE_3_LAYERS_ONLY
#endif
#if (!defined(_4LAYERS) || defined(RTP_TRIPLANAR) || defined(RTP_USE_COLOR_ATLAS)) && defined(FORCE_ANISO)
	#undef FORCE_ANISO
#endif
...
struct Input {
	#ifdef RTP_PLANET
		float2 cylinderMap : TEXCOORD0; // texcoords from mesh (mapped cylindrical)
	#else
		#ifdef RTP_STANDALONE
			float2 texCoords : TEXCOORD0; // worldpos.xz
		#else
			float2 uv_Control : TEXCOORD0; // texcoords from mesh
		#endif	
	#endif
	float4 _uv_Relief;
	float4 _uv_Aux;
...
float AtmosphereDistance(in Input IN) {
...
float _Fstart,_Fend;
	...
	void customFog (Input IN, RTPSurfaceOutput o, inout fixed4 color) {
		#ifdef RTP_PLANET
			float realFogDistance=(IN._uv_Relief.w-max(0.0,AtmosphereDistance(IN)));
		...
		#else		
			#ifdef RTP_PLANET
				float lightScatterValue=IN.lightDir.z*2+1; // lightDir.z for planet has dot(light, surface normal)
				f=lerp(f, 1, saturate( (IN.lightDir.w-_PlanetRadius) / _PlanetAtmosphereLength ) );
				float3 _FColorBack=lerp( lerp(color.rgb, _FColor3.rgb, _FColor3.a), _FColor2, saturate(lightScatterValue));
				color.rgb=lerp( lerp(_FColorBack, _FColor, saturate(IN.lightDir.z*2)), color.rgb, f);
			#else
...
void vert (inout appdata_full v, out Input o) {
		...
		float3 worldNormal=normalize(mul(_Object2World, float4(SCALED_NORMAL,0)).xyz);
			...
			// RTP standalone shading
					...
					o._uv_Relief.xy=v.vertex.xz /  _TERRAIN_ReliefTransformTriplanarZ / unity_Scale.w;
				#else
					o._uv_Relief.xy=Wpos.xz /  _TERRAIN_ReliefTransformTriplanarZ;
				#endif
			#else
				o._uv_Relief.xy=v.texcoord.xy * _TERRAIN_ReliefTransform.xy + _TERRAIN_ReliefTransform.zw;
			#endif
					...
					#if defined(LOCAL_SPACE_UV)
						o._uv_Relief.z=v.vertex.y / _TERRAIN_ReliefTransformTriplanarZ / unity_Scale.w;
					#else
		...
		float _distance=length(_Dir);
			...
			float3 refl_vec = normalize(mul((float3x3)_Object2World, viewRefl)).xyz*0.99;
			float3 worldRefl;
			...
			float3 verticalDirObjSpace=normalize(v.vertex.xyz);
			{
		...
		#if defined(APPROX_TANGENTS) || defined(RTP_TRIPLANAR)
	...
	//		   	//v.tangent.xy=normalize(v.tangent.xy); // z normalizacja roznica nieznaczna ...
			...
			float3x3 rotation = float3x3( v.tangent.xyz, binormal, v.normal.xyz );
				...
				o.lightDir.xyz = mul (rotation, objSpaceLDir);
			...
			#else
				#ifdef RTP_PLANET
					// pozycja vertexa determinuje pion
					float3 planetCenter=float3(_Object2World[0][3], _Object2World[1][3], _Object2World[2][3]);
					o._viewDir.zw = normalEncode(( mul (rotation, mul(_World2Object, float4(Wpos-planetCenter ,0)).xyz) ).xyz);
				#else
			...
			#if defined(COLOR_EARLY_EXIT) || !defined(APPROX_TANGENTS)
				#ifdef RTP_PLANET
					o.lightDir.w = length(v.vertex.xyz);
				#else
					o.lightDir.w = Wpos.y;
				#endif
			#else
		...
		// terravol LOD vertices interpolation
			...
			float lod_far = saturate((o._uv_Relief.w - _TERRAIN_distance_start_vertexinterpolation) / _TERRAIN_distance_transition_vertexinterpolation);
...
void surf (Input IN, inout RTPSurfaceOutput o) { // RTPSurfaceOutput
	...
	#ifdef RTP_PLANET
		#define IN_uv_Control (IN.cylinderMap)
		o.RTP.y=saturate(IN.lightDir.z+0.1); //  ocieniona strona planety jets zawsze w cieniu
	#else
		#ifdef GEOM_BLEND
			float2 globalUV=(IN.texCoords-_TERRAIN_PosSize.xy)/_TERRAIN_PosSize.zw;
...
//			#endif
		...
		// (zmienione obl. global normal)
			...
			float4 pixel_trees_val=tex2D(_TreesMapGlobal, IN_uv_Control);
		...
		#if defined(RTP_AMBIENT_EMISSIVE_MAP)
			float4 eMapVal=tex2D(_AmbientEmissiveMapGlobal, IN_uv_Control);
	...
	float2 mip_selector;
		...
		//
		#ifndef RTP_PLANET
			float2 uv_Control=(IN.texCoords-_TERRAIN_PosSize.xy)/_TERRAIN_PosSize.zw;
		#endif
			...
			#endif
		#else
			float4 splat_controlA = tex2D(_Control1, uv_Control);
		#endif
		...
		#else
			float4 splat_controlB = tex2D(_Control2, IN_uv_Control);
	...
	IN_viewDir.z=sqrt(1 - saturate(dot(IN_viewDir.xy, IN_viewDir.xy)));	
		...
		fixed4 hMapVal_YZ = tex2D(_TERRAIN_HeightMap, IN._uv_Relief.yz);
		fixed4 hMapVal_XY = tex2D(_TERRAIN_HeightMap, IN._uv_Relief.xy);
		fixed4 hMapVal_XZ = tex2D(_TERRAIN_HeightMap, IN._uv_Relief.xz);
		...
		float global_color_blend=lerp( lerp(_GlobalColorMapBlendValues.y, _GlobalColorMapBlendValues.x, _uv_Relief_z*_uv_Relief_z), _GlobalColorMapBlendValues.z, _uv_Relief_w);
				...
				float4 global_color_value=tex2D(_ColorMapGlobal, uv_Control);
			#else
				// przy rtp_planet robimy distort nawet w simple (poprawia znacznie rozdzielczosc global colormapy)
				float4 global_color_value=tex2D(_ColorMapGlobal, uv_Control+(global_bump_val.rg-float2(0.5f, 0.5f))*_GlobalColorMapDistortByPerlin);
			#endif
		#else
			#if defined(RTP_PLANET) && defined(RTP_COLORMAP_COVERAGE)
				// wyliczone na początku aby można było sterować coveragem
			#else
				#if defined(RTP_SIMPLE_SHADING) || defined(COLOR_EARLY_EXIT)
					float4 global_color_value=tex2D(_ColorMapGlobal, IN_uv_Control);
				#else
					float4 global_color_value=tex2D(_ColorMapGlobal, IN_uv_Control+(global_bump_val.rg-float2(0.5f, 0.5f))*_GlobalColorMapDistortByPerlin);
				#endif
			#endif
		#endif
		...
		#if defined(RTP_STANDALONE) && !defined(RTP_PLANET)
			global_color_value.rgb=lerp(tex2Dlod(_ColorMapGlobal, float4(uv_Control+(global_bump_val.rg-float2(0.5f, 0.5f))*_GlobalColorMapDistortByPerlin, _GlobalColorMapNearMIP.xx)).rgb, global_color_value.rgb, _uv_Relief_w);
		#else
	...
	diffFresnel*=diffFresnel;
		...
		#else
			#if defined(RTP_STANDALONE) && !defined(RTP_PLANET)
				float water_mask=tex2Dlod(_BumpMapGlobal, float4(uv_Control*(1-2*_BumpMapGlobal_TexelSize.xx)+_BumpMapGlobal_TexelSize.xx, mip_selector_tmp.xx)).b;
			#else
	...
	float2 IN_uv_Relief_Offset;
		...
		// zakładam, że jak ktoś chce mieć b. precyzyjny śnieg (kolor+normalne to skoryguję mu też parallax offset)
			...
			// detail heightmapę bieżemy z tej warstwy co normalmapę
				...
				float4 SNOW_PM_OFFSET=float4( ParallaxOffset(tHA.x, _TERRAIN_ExtrudeHeight*_uv_Relief_z*COLOR_DAMP_VAL, IN_viewDir.xyz) ,0,0);
			#endif
			#if defined(RTP_SNW_CHOOSEN_LAYER_NORM_1)
				float4 SNOW_PM_OFFSET=float4( ParallaxOffset(tHA.y, _TERRAIN_ExtrudeHeight*_uv_Relief_z*COLOR_DAMP_VAL, IN_viewDir.xyz) ,0,0);
			#endif
			#if defined(RTP_SNW_CHOOSEN_LAYER_NORM_2)
				float4 SNOW_PM_OFFSET=float4( ParallaxOffset(tHA.z, _TERRAIN_ExtrudeHeight*_uv_Relief_z*COLOR_DAMP_VAL, IN_viewDir.xyz) ,0,0);
			#endif
			#if defined(RTP_SNW_CHOOSEN_LAYER_NORM_3)
				float4 SNOW_PM_OFFSET=float4( ParallaxOffset(tHA.w, _TERRAIN_ExtrudeHeight*_uv_Relief_z*COLOR_DAMP_VAL, IN_viewDir.xyz) ,0,0);
			#endif
		#else
	...
	float4 MIPmult0123=_MIPmult0123*_uv_Relief_w;
		...
		half4 gloss_aniso=0;
		fixed4 c;
		c = tex2D(_SplatA0, IN._uv_Relief.xy); col_aniso = splat_control1.x * c.rgb; gloss_aniso.r = c.a;
		c = tex2D(_SplatA1, IN._uv_Relief.xy); col_aniso += splat_control1.y * c.rgb; gloss_aniso.g = c.a;
		#if !defined(USE_2_LAYERS_ONLY)
		c = tex2D(_SplatA2, IN._uv_Relief.xy); col_aniso += splat_control1.z * c.rgb; gloss_aniso.b = c.a;
		#if !defined(USE_3_LAYERS_ONLY)
		c = tex2D(_SplatA3, IN._uv_Relief.xy); col_aniso += splat_control1.w * c.rgb; gloss_aniso.a = c.a;			
		#endif
		#endif		
		fixed4 normals_combined = tex2D(_BumpMap01, IN._uv_Relief.xy).rgba*splat_control1.rrgg;
		#if !defined(USE_2_LAYERS_ONLY)
		normals_combined+=tex2D(_BumpMap23, IN._uv_Relief.xy).rgba*splat_control1.bbaa;
		#endif		
		float3 n_aniso;
...
//#endif		 	
			...
			float glcombined = dot(gloss, splat_control1);
				...
				uvSplat01M=frac(IN._uv_Relief.xyxy*MixScaleRouted0123.xxyy)*_multMix0123+_offMix0123;
				uvSplat01M.zw+=float2(0.5,0);
				uvSplat23M=frac(IN._uv_Relief.xyxy*MixScaleRouted0123.zzww)*_multMix0123+_offMix0123;
...
//#endif		 			
			...
			float4 gloss=0;
				...
				if (triplanar_blend_flag) {
					...
					float4 tmp_gloss=0;
						...
						#endif							
					#else
						c = tex2Dlod(_SplatA0, uvTRI_tmp.xyzz); col += c.rgb*splat_control1.x; tmp_gloss.r=c.a;
						c = tex2Dlod(_SplatA1, uvTRI_tmp.xyzz); col += c.rgb*splat_control1.y; tmp_gloss.g=c.a;
						#if !defined(USE_2_LAYERS_ONLY)
						c = tex2Dlod(_SplatA2, uvTRI_tmp.xyzz); col += c.rgb*splat_control1.z; tmp_gloss.b=c.a;
						#if !defined(USE_3_LAYERS_ONLY)
						c = tex2Dlod(_SplatA3, uvTRI_tmp.xyzz); col += c.rgb*splat_control1.w; tmp_gloss.a=c.a;
						#endif
						#endif			
					#endif						
					gloss=tmp_gloss;
			...
			float glcombined = dot(gloss, splat_control1);
					...
					uvSplat01M=frac(IN._uv_Relief.xyxy*MixScaleRouted0123.xxyy)*_multMix+_offMix;
					uvSplat01M.zw+=float2(0.5,0);
					uvSplat23M=frac(IN._uv_Relief.xyxy*MixScaleRouted0123.zzww)*_multMix+_offMix;
					uvSplat23M+=float4(0,0.5,0.5,0.5);	
					...
					float4 uvSplat01M=IN._uv_Relief.xyxy*MixScaleRouted0123.xxyy;
					float4 uvSplat23M=IN._uv_Relief.xyxy*MixScaleRouted0123.zzww;
...
//#endif		
			...
			float4 gloss=0;			
				...
				float4 tmp_gloss=0;
					...
					#endif							
				#else
					c = tex2Dlod(_SplatA0, float4(uvTRI.xy,_MixMipActual.xx)); col += c.rgb*splat_control1.x; tmp_gloss.r=c.a;
					c = tex2Dlod(_SplatA1, float4(uvTRI.xy,_MixMipActual.yy)); col += c.rgb*splat_control1.y; tmp_gloss.g=c.a;
					#if !defined(USE_2_LAYERS_ONLY)
					c = tex2Dlod(_SplatA2, float4(uvTRI.xy,_MixMipActual.zz)); col += c.rgb*splat_control1.z; tmp_gloss.b=c.a;
					#if !defined(USE_3_LAYERS_ONLY)
					c = tex2Dlod(_SplatA3, float4(uvTRI.xy,_MixMipActual.ww)); col += c.rgb*splat_control1.w; tmp_gloss.a=c.a;
					#endif
					#endif		
				#endif						
				gloss=tmp_gloss;					
...
//#endif		
	...
	#if defined(RTP_UV_BLEND) && defined(RTP_NORMALS_FOR_REPLACE_UV_BLEND)
	{
			...
			float4 mipN=mip_selector.xxxx+rtp_mipoffset_bump+MIPmult0123+log2(MixScaleRouted0123);
				...
				mipN += snow_depth;
			#endif
			n.xy = tex2Dlod(_BumpMap01, float4(IN._uv_Relief.xy*MixScaleRouted0123.x,  mipN.xx)).rg*splat_control1.r;
			n.xy += tex2Dlod(_BumpMap01, float4(IN._uv_Relief.xy*MixScaleRouted0123.y,  mipN.yy)).ba*splat_control1.g;
			n.xy += tex2Dlod(_BumpMap23, float4(IN._uv_Relief.xy*MixScaleRouted0123.z,  mipN.zz)).rg*splat_control1.b;
			n.xy += tex2Dlod(_BumpMap23, float4(IN._uv_Relief.xy*MixScaleRouted0123.w,  mipN.ww)).ba*splat_control1.a;
			#ifndef _4LAYERS
	...
	o_Gloss=lerp( saturate(o_Gloss+4*(dot(splat_control1_nobleed, _FarSpecCorrection0123) + dot(splat_control2_nobleed, _FarSpecCorrection4567))), o_Gloss, (1-_uv_Relief_w)*(1-_uv_Relief_w) );
	...
	#else
		#ifdef RTP_IBL_DIFFUSE
			half3 colBrightnessNotAffectedByColormap=col;
		#endif
	#endif
		...
		col=lerp( col, rtp_snow_color_tex, snow_val );
			...
			float3 n;
				...
				n.xy=tex2Dlod(_BumpMap01, float4(IN_uv_Relief_Offset.xy+SNOW_PM_OFFSET, mip_selector + rtp_mipoffset_bump)).rg*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_1
				n.xy=tex2Dlod(_BumpMap01, float4(IN_uv_Relief_Offset.xy+SNOW_PM_OFFSET, mip_selector + rtp_mipoffset_bump)).ba*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_2
				n.xy=tex2Dlod(_BumpMap23, float4(IN_uv_Relief_Offset.xy+SNOW_PM_OFFSET, mip_selector + rtp_mipoffset_bump)).rg*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_3
				n.xy=tex2Dlod(_BumpMap23, float4(IN_uv_Relief_Offset.xy+SNOW_PM_OFFSET, mip_selector + rtp_mipoffset_bump)).ba*2-1;
			#endif
	...
	// przeniesione pod emisję (która zależy od specular materiału _pod_ śniegiem)
		...
		value*=saturate(1 - _EmissionFromColormapAlphaLightDependent*(IN.lightDir.z-0.15));
		#ifdef RTP_SNOW
...
//	return;
	...
	#ifdef RTP_TREESGLOBAL	
		#if defined(RTP_STANDALONE) && !defined(RTP_PLANET)
			float4 pixel_trees_val=tex2D(_TreesMapGlobal, uv_Control);
		#else
			float4 pixel_trees_val=tex2D(_TreesMapGlobal, IN_uv_Control);
		#endif
		float pixel_trees_blend_val=saturate((pixel_trees_val.r+pixel_trees_val.g+pixel_trees_val.b)*_TERRAIN_trees_pixel_values.z);
	...
	#if defined(RTP_AMBIENT_EMISSIVE_MAP)
		#if defined(RTP_STANDALONE) && !defined(RTP_PLANET)
			float4 eMapVal=tex2D(_AmbientEmissiveMapGlobal, uv_Control);
		#else
			float4 eMapVal=tex2D(_AmbientEmissiveMapGlobal, IN_uv_Control);
		#endif
		...
		#ifdef RTP_STANDALONE
			float pixel_trees_shadow_val=saturate((IN._uv_Relief.w - _shadow_distance_start) / _shadow_distance_transition);
			pixel_trees_shadow_val=lerp(1, eMapVal.a, pixel_trees_shadow_val);
	...
	float o_SpecularInvSquared = (1-o.Specular)*(1-o.Specular);
		...
		#define RTP_IBLSPEC_BUMPED
			...
			#else
				float n_dot_v = saturate(dot (IBLNormal, normalize(IN.viewDir.z<0? -IN.viewDir.xyz:IN.viewDir.xyz)));
			#endif
	...
	// przeniesione, norm_edge zamienione na o.Normal w obl. reflection
		...
		float3 global_norm;
				...
				global_norm.xy=tex2D(_NormalMapGlobal, uv_Control).xy * 2 - 1;
			#else
				global_norm.xy=tex2D(_NormalMapGlobal, uv_Control).wy * 2 - 1;
			#endif
		#else
			#if defined(SHADER_API_GLES) && defined(SHADER_API_MOBILE)
		...
		#endif
		#ifdef RTP_PLANET
			//global_norm.xy*=_NormalMapGlobalStrength;
		#else
		...
		global_norm.z=sqrt(1 - saturate(dot(global_norm.xy, global_norm.xy)));
			...
			//return;
			//
			o.RTP.y*=lerp( 1, dot(IN.lightDir, global_norm)*0.5+0.5, saturate(IN._uv_Relief.w/_NormalMapGlobalFarOnly)*_NormalMapGlobalStrength );
		#else
	...
	// used on triplanar standalone shader as geom blend
			...
			float4 terrain_coverage=tex2D(_TERRAIN_Control, IN_uv_Control);
			float2 tiledUV=(IN.texCoords-_TERRAIN_PosSize.xy+_TERRAIN_Tiling.zw)/_TERRAIN_Tiling.xy;
			float4 splat_control1HB=terrain_coverage * tex2D(_TERRAIN_HeightMap, tiledUV) * IN.color.a;

...
//#endif
		...
		{ EditorGUILayout.BeginVertical("Box");
			...
			EditorGUILayout.BeginHorizontal();
				...
				GUI.color=skin_color;					
				_target.RTP_COLORSPACE_LINEAR=EditorGUILayout.Toggle(_target.RTP_COLORSPACE_LINEAR, GUILayout.MinWidth(20), GUILayout.MaxWidth(20));
				RTPFogUpdate fUpdater=_target.GetComponent(typeof(RTPFogUpdate)) as RTPFogUpdate;
				if (fUpdater) fUpdater.LinearColorSpace=_target.RTP_COLORSPACE_LINEAR;
			EditorGUILayout.EndHorizontal();
			...
			EditorGUILayout.BeginHorizontal();
				EditorGUILayout.LabelField("Basic PBL fit for deferred", GUILayout.MinWidth(170), GUILayout.MaxWidth(170));
		...
		if (_target.SHADER_USAGE_FirstPass || _target.SHADER_USAGE_Terrain2Geometry) {
			...
			samplers_used+=_target.RTP_NORMALGLOBAL ? 1:0;
		...
		if (_target.show_first_features) {
			...
			if (_target.RTP_4LAYERS_MODE) {
				...
				if (!_target.RTP_SUPER_SIMPLE) {
					...
					EditorGUILayout.HelpBox("You can force using aniso filtering for basic detail textures (with their normalmaps). This option can cost a bit and is constrained by 8 layers mode, atlasing or triplanar (no aniso filtering available then).",MessageType.None, true);
					if (!_target.RTP_4LAYERS_MODE || _target.RTP_USE_COLOR_ATLAS_FIRST || _target.RTP_TRIPLANAR_FIRST) {
						_target.FORCE_ANISO_FIRST=false;
					}
					EditorGUI.BeginDisabledGroup(!_target.RTP_4LAYERS_MODE || _target.RTP_USE_COLOR_ATLAS_FIRST || _target.RTP_TRIPLANAR_FIRST);
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("Basic aniso filtering", GUILayout.MinWidth(160), GUILayout.MaxWidth(160));
					_target.FORCE_ANISO_FIRST=EditorGUILayout.Toggle(_target.FORCE_ANISO_FIRST);
					EditorGUILayout.EndHorizontal();
					EditorGUI.EndDisabledGroup();
				}
			}
...
//					EditorGUILayout.HelpBox("", MessageType.None, true);
					...
					EditorGUI.BeginDisabledGroup(!_target.RTP_REFLECTION_FIRST);
						...
						EditorGUILayout.EndHorizontal();
					EditorGUI.EndDisabledGroup();
					if (_target.RTP_IBL_DIFFUSE_FIRST) {
						_target.RTP_RECONSTRUCT_WORLDNORMAL_FIRST=true;
					}
					EditorGUI.BeginDisabledGroup(!_target.RTP_REFLECTION_FIRST || _target.RTP_IBL_DIFFUSE_FIRST);
						EditorGUILayout.BeginHorizontal();
							EditorGUILayout.LabelField("  Exact norm / refl vectors", GUILayout.MinWidth(160), GUILayout.MaxWidth(160));
							_target.RTP_RECONSTRUCT_WORLDNORMAL_FIRST=EditorGUILayout.Toggle(_target.RTP_RECONSTRUCT_WORLDNORMAL_FIRST);
						EditorGUILayout.EndHorizontal();
					EditorGUI.EndDisabledGroup();
										
					EditorGUILayout.BeginHorizontal();
						EditorGUILayout.LabelField("IBL diffuse cubemap", GUILayout.MinWidth(160), GUILayout.MaxWidth(160));
					...
					EditorGUILayout.EndHorizontal();
					EditorGUI.BeginDisabledGroup(!_target.RTP_IBL_SPEC_FIRST || _target.RTP_IBL_DIFFUSE_FIRST);
						EditorGUILayout.BeginHorizontal();
							EditorGUILayout.LabelField("  Exact norm / refl vectors", GUILayout.MinWidth(160), GUILayout.MaxWidth(160));
							_target.RTP_RECONSTRUCT_WORLDNORMAL_FIRST=EditorGUILayout.Toggle(_target.RTP_RECONSTRUCT_WORLDNORMAL_FIRST);
						EditorGUILayout.EndHorizontal();
					EditorGUI.EndDisabledGroup();
					if (!_target.RTP_IBL_SPEC_FIRST && !_target.RTP_IBL_DIFFUSE_FIRST && !_target.RTP_REFLECTION_FIRST) {
						_target.RTP_RECONSTRUCT_WORLDNORMAL_FIRST=false;
					}
				// IBL / Reflections
				EditorGUILayout.EndVertical(); }
...
//		
		...
		if (_target.show_add_features) {
			...
			if (!_target.RTP_SUPER_SIMPLE) {
				...
				EditorGUILayout.HelpBox("You can force using aniso filtering for basic detail textures (with their normalmaps). This option can cost a bit and is constrained by atlasing and triplanar (no aniso filtering available then).",MessageType.None, true);
				if (_target.RTP_USE_COLOR_ATLAS_ADD || _target.RTP_TRIPLANAR_ADD) {
					_target.FORCE_ANISO_ADD=false;
				}
				EditorGUI.BeginDisabledGroup(_target.RTP_USE_COLOR_ATLAS_ADD || _target.RTP_TRIPLANAR_ADD);
					EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("Basic aniso filtering", GUILayout.MinWidth(160), GUILayout.MaxWidth(160));
					_target.FORCE_ANISO_ADD=EditorGUILayout.Toggle(_target.FORCE_ANISO_ADD);
					EditorGUILayout.EndHorizontal();
				EditorGUI.EndDisabledGroup();
...
//					EditorGUILayout.HelpBox("", MessageType.None, true);
					...
					EditorGUI.BeginDisabledGroup(!_target.RTP_REFLECTION_ADD);
						...
						EditorGUILayout.EndHorizontal();
					EditorGUI.EndDisabledGroup();
					if (_target.RTP_IBL_DIFFUSE_ADD) {
						_target.RTP_RECONSTRUCT_WORLDNORMAL_ADD=true;
					}
					EditorGUI.BeginDisabledGroup(!_target.RTP_REFLECTION_ADD || _target.RTP_IBL_DIFFUSE_ADD);
						EditorGUILayout.BeginHorizontal();
							EditorGUILayout.LabelField("  Exact norm / refl vectors", GUILayout.MinWidth(160), GUILayout.MaxWidth(160));
							_target.RTP_RECONSTRUCT_WORLDNORMAL_ADD=EditorGUILayout.Toggle(_target.RTP_RECONSTRUCT_WORLDNORMAL_ADD);
						EditorGUILayout.EndHorizontal();
					EditorGUI.EndDisabledGroup();
					...
					EditorGUILayout.EndHorizontal();
					EditorGUI.BeginDisabledGroup(!_target.RTP_IBL_SPEC_ADD || _target.RTP_IBL_DIFFUSE_ADD);
						EditorGUILayout.BeginHorizontal();
							EditorGUILayout.LabelField("  Exact norm / refl vectors", GUILayout.MinWidth(160), GUILayout.MaxWidth(160));
							_target.RTP_RECONSTRUCT_WORLDNORMAL_ADD=EditorGUILayout.Toggle(_target.RTP_RECONSTRUCT_WORLDNORMAL_ADD);
						EditorGUILayout.EndHorizontal();
					EditorGUI.EndDisabledGroup();
					if (!_target.RTP_IBL_SPEC_ADD && !_target.RTP_IBL_DIFFUSE_ADD && !_target.RTP_REFLECTION_ADD) {
						_target.RTP_RECONSTRUCT_WORLDNORMAL_ADD=false;
					}
				// IBL / Reflections
				EditorGUILayout.EndVertical(); }
...
//#endif
			...
			// force aniso
			ChangeShaderDef(ref _code, "FORCE_ANISO", AddPass_flag ? _target.FORCE_ANISO_ADD : _target.FORCE_ANISO_FIRST);
			...
			// reconstruct world norm / refl
			ChangeShaderDef(ref _code, "RTP_RECONSTRUCT_WORLDNORMAL", AddPass_flag ? _target.RTP_RECONSTRUCT_WORLDNORMAL_ADD : _target.RTP_RECONSTRUCT_WORLDNORMAL_FIRST);
	...
	private void SyncFeaturesFromFile(string shader_path, bool shader_flag, bool addpass_flag) {
		...
		if (System.IO.File.Exists(shader_path)) {
			...
			if (addpass_flag) {		
				...
				if (CheckDefine(_code, "//#define FORCE_ANISO")) _target.FORCE_ANISO_ADD=false;
				else if (CheckDefine(_code, "#define FORCE_ANISO")) _target.FORCE_ANISO_ADD=true;
				...
				if (CheckDefine(_code, "//#define RTP_RECONSTRUCT_WORLDNORMAL")) _target.RTP_RECONSTRUCT_WORLDNORMAL_ADD=false;
				else if (CheckDefine(_code, "#define RTP_RECONSTRUCT_WORLDNORMAL")) _target.RTP_RECONSTRUCT_WORLDNORMAL_ADD=true;
				...
				if (CheckDefine(_code, "//#define FORCE_ANISO")) _target.FORCE_ANISO_FIRST=false;
				else if (CheckDefine(_code, "#define FORCE_ANISO")) _target.FORCE_ANISO_FIRST=true;
				...
				if (CheckDefine(_code, "//#define RTP_RECONSTRUCT_WORLDNORMAL")) _target.RTP_RECONSTRUCT_WORLDNORMAL_FIRST=false;
				else if (CheckDefine(_code, "#define RTP_RECONSTRUCT_WORLDNORMAL")) _target.RTP_RECONSTRUCT_WORLDNORMAL_FIRST=true;

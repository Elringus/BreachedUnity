...
public class ReliefTerrainEditor : Editor {
	...
	void OnEnable() {
		...
		if (_target.globalSettingsHolder!=null && !_target.globalSettingsHolder.dont_check_for_interfering_terrain_replacement_shaders) {
			...
			int res=-1;
			if (repl_firstPass!="" && repl_firstPass.IndexOf ("ReliefTerrain")==-1) {
			...
			if (res==-1 && repl_addPass!="" && repl_addPass.IndexOf ("ReliefTerrain")==-1) {
				res=EditorUtility.DisplayDialogComplex("RTP Warning", "Interfering terrain replacement shader found in your project ! Unless you're using materials on RTP terrain you should REMOVE another shaders.","Show me","Cancel", "Dont' bother me again");
				if (res==0) {
	...
	public override void OnInspectorGUI () {
			...
			if (_target.show_settings) {
				...
				case ReliefTerrainSettingsItems.MainSettings:
					...
					if (terrainComp) {
						...
						if ((_targetRT.renderer.lightmapIndex!=255) && (_targetRT.renderer.lightmapIndex!=-1) && LightmapSettings.lightmapsMode!=LightmapsMode.Directional) {
							showLightmapShading=true;
						}
					}
				...
				case ReliefTerrainSettingsItems.GlobalColor:
					...
					if (!_target.paint_flag) {
						...
						if (!_targetRT.collider || !_targetRT.collider.enabled) _target.paint_flag=false;
						GUI.color=c;
					...
					if (_target.paint_flag) {
						if (!_targetRT.collider || !_targetRT.collider.enabled) EditorGUILayout.HelpBox("Object doesn't have collider (necessary for painting).",MessageType.Error, true);
		...
		public void OnSceneGUI() {
			...
			if(Event.current.type==EventType.keyDown && Event.current.keyCode==KeyCode.L) {
				current.Use();
				if (_targetRT.collider) {
					Ray ray = HandleUtility.GUIPointToWorldRay(Event.current.mousePosition);
					paintHitInfo_flag=_targetRT.collider.Raycast(ray, out paintHitInfo, Mathf.Infinity);

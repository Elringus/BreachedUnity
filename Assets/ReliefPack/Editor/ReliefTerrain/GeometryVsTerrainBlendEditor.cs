				...
				if (GUILayout.Button(new GUIContent("Stick to Blended Object along normals"))) {
					for(int i=0; i<targets.Length; i++) {
						...
						if (mft!=null) {						
							...
							_atarget.renderer.castShadows=false;
							_atarget.SetupValues();
							EditorUtility.SetDirty(targets[i]);
						}						
					}
...
//			}
			...
			if (_target.paint_flag) {
				...
				if (_target.paint_mode<1 && (!_target.collider || !_target.collider.enabled)) EditorGUILayout.HelpBox("Object doesn't have collider (necessary for painting).",MessageType.Error, true);
				...
				int npaint_mode;
				if (_target.Sticked) {
					if (_target.blendedObject && _target.blendedObject.collider) {// && (_target.blendedObject.collider is TerrainCollider)) {
			...
			case EventType.mouseMove:
				...
				if (_target.RealizePaint_Flag && _target.ModifyTris) {
					if (_target.BuildMeshFlag) {
						...
						if (_target.collider && _target.collider is MeshCollider) {
							if (mf) {
								(_target.collider as MeshCollider).sharedMesh=mf.sharedMesh;
							}
						}				
	...
	private void get_paint_coverage(bool shift_flag) {
		...
		GeometryVsTerrainBlend _target=(GeometryVsTerrainBlend)target;
		if (_target.Sticked) {
			if (!_target.blendedObject || !_target.blendedObject.collider) return;
		} else {
			if (!_target.collider) return;
		}
		if (_target.paint_mode<1) {
				...
				if (_target.BuildMeshFlag) {
					...
					if (terrainComp) {
						...
						if ((geomScript==null) || geomScript.controlA!=null) {
							...
							if (geomScript!=null) {
								cellW=_target.blendedObject.renderer.bounds.size.x/(geomScript.controlA.width)*Mathf.Pow(2.0f, -1.0f*_target.addTrisSubdivision);
								cellH=_target.blendedObject.renderer.bounds.size.z/(geomScript.controlA.height)*Mathf.Pow(2.0f, -1.0f*_target.addTrisSubdivision);
							} else {
...
//								}								
							...
							Vector3 pnt=GetWorldPointFromMouse(terrainCollider);
							Vector3 terrainPosition=_target.blendedObject.renderer.bounds.min;
							int centerX=Mathf.RoundToInt( (pnt.x-terrainPosition.x)/cellW );
							int centerZ=Mathf.RoundToInt( (pnt.z-terrainPosition.z)/cellH );
							for(int i=0; i<_w; i++) {
								for(int j=0; j<_h; j++) {
									...
									RaycastHit hit;
									if (terrainCollider.Raycast(new Ray(new Vector3(cover_verts[cover_num].x, _target.blendedObject.renderer.bounds.max.y, cover_verts[cover_num].z), paintSurfaceNormal), out hit, Mathf.Infinity)) {
					...
					if (tris!=null) {
						Vector3 pnt;
						if (_target.Sticked) {
							pnt=GetWorldPointFromMouse(_target.blendedObject.collider);
						} else {
							pnt=GetWorldPointFromMouse(_target.collider);
						}
						cover_num=0;
	        ...
	        Ray ray = HandleUtility.GUIPointToWorldRay(Event.current.mousePosition);
			if (_target.blendedObject && _target.blendedObject.collider) {
				paintHitInfo_flag=_target.blendedObject.collider.Raycast(ray, out paintHitInfo, Mathf.Infinity);
	...
	private void CheckShaderForBlendCapability(GeometryVsTerrainBlend _target=null) {
		...
		if (_target.renderer.sharedMaterial) {
			Shader shad=_target.renderer.sharedMaterial.shader;

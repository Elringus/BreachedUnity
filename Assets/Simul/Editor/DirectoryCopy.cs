...
namespace simul
	...
	public class DirectoryCopy
	{
		public static void CopyPluginsAndGizmosToAssetsFolder()
		{
			...
			DirectoryCopy.Copy(assetsPath + s + "Simul" + s + "Gizmos", gizmosPath, true, true, true);
			// 2. The plugins folder
			String pluginsPath = assetsPath + s + "Plugins";
			DirectoryCopy.Copy(assetsPath + s + "Simul" + s + "Plugins", pluginsPath, true, true, true);
		}
		public static void Copy(string sourceDirName, string destDirName, bool copySubDirs, bool skipMetas, bool newerOnly = false, bool report = false)
		{
			...
			if (copySubDirs)
			{
				foreach (DirectoryInfo subdir in dirs)
				{
					...
					Copy(subdir.FullName, temppath, copySubDirs, skipMetas, newerOnly, report);
				}
			}
		}
	}
